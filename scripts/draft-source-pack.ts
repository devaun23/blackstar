...
async function queryEvidence(topic: string): Promise<EvidenceItem[]> {
  // Primary: query by topic_tags (items tagged during ingestion)
  ...
  // Fallback: text search on section_heading and claim for topics not yet re-ingested.
  // This catches items that were tagged under a broader topic (e.g., "Acute Kidney Injury"
  // for hyponatremia items) before the topic map was expanded.
  console.log(`  → No topic_tags match. Falling back to text search on section_heading...`);
  ...
  const { data: fallbackData, error: fallbackError } = await supabase
    .from('di_evidence_item')
    .select('display_id, item_type, claim, section_heading, source, trigger_presentation, association, differential')
    .or(`section_heading.ilike.%${searchPattern}%,claim.ilike.%${searchPattern}%`)
    .order('display_id')
    .limit(200);
  ...
  if (fallbackData && fallbackData.length > 0) {
    console.log(`  → Text search found ${fallbackData.length} items`);
  }

  return (fallbackData ?? []) as EvidenceItem[];
}
